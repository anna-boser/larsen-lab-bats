...
bat_exp <- rast(here("data/landcover_analysis/bats/bats_exponential_corrected_2018.tif")) 
...
# The crude solution to the above problem is to extract the mean and standard deviation separately
extract_expBats_terra_NewCode_mean <- cbind(fveg_landIQ_labeled_summ_vect, terra::extract(bat_exp_NAD83, fveg_landIQ_labeled_summ_vect, fun = mean, na.rm=TRUE))

extract_expBats_terra_NewCode_sd <- cbind(fveg_landIQ_labeled_summ_vect, terra::extract(bat_exp_NAD83, fveg_landIQ_labeled_summ_vect, fun = sd, na.rm=TRUE))


# convert the results into mean and sd data frames
extract_expBats_terra_NewCode_mean_df <- data.frame(extract_expBats_terra_NewCode_mean)
extract_expBats_terra_NewCode_sd_df <- data.frame(extract_expBats_terra_NewCode_sd)

# Rename the extracted column to be "Mean" instead of the generic "bats_exponential_corrected_2018"
extract_expBats_terra_NewCode_mean_df <- extract_expBats_terra_NewCode_mean_df %>% 
  rename(Mean = bats_exponential_corrected_2018)
# Rename the extracted column to be "SD" instead of the generic "bats_exponential_corrected_2018"
extract_expBats_terra_NewCode_sd_df <- extract_expBats_terra_NewCode_sd_df %>% 
  rename(SD = bats_exponential_corrected_2018)

# merge into a single data frame that contains the extracted Mean and SD data
extract_expBats_terra_NewCode_df <- merge(extract_expBats_terra_NewCode_mean_df, extract_expBats_terra_NewCode_sd_df, by = c("NewCode", "ID"))
...
extract_expBats_simple <- extract_expBats_terra_NewCode_df %>% 
...
extract_merge_obs_mergecolor <- merge(extract_merge_obs, group_colormap, by = "NewCode")
```
