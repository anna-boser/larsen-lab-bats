...
LC_value_labels <- read_csv("~/Documents/github/larsen-lab-bats/data/landcover_analysis/CSV_files/SummerCropID_fvegid_combo.csv")
```
...
extract_rawbats_zonalstats <-  exact_extract(bat_occ, lc_label_sf, fun = c('mean', 'stdev'))
```


```{r}
extract_rawbats_zonalstats <- extract_rawbats_zonalstats %>%
...
```{r}
lc_label_zonalstats <- merge(lc_label_sf, extract_rawbats_zonalstats, by = "ID")
```

```{r}
st_write(lc_label_zonalstats, "~/Documents/github/larsen-lab-bats/data/landcover_analysis/output_layers/lc_label_zonalstats.shp", append = F)
```
...
```
```{r}
lc_area_GenLC_summ <- lc_label_zonalstats %>% 
  st_drop_geometry() %>% 
  group_by(Gen_Class) %>% 
  summarise(area_ha = sum(area_m2/1e4, na.rm = T))
```
...
```{r}
ggplot(data = lc_area_GenLC_summ, aes(x = reorder(Gen_Class, area_ha), y = area_ha)) +
  geom_col() + 
  coord_flip() +
  labs(x = "Land Cover", y = "Area in the Study Region (ha)") +
  theme_bw() +
...
```{r}
lc_rawbats_GenLCsumm <- lc_label_zonalstats %>% 
  st_drop_geometry() %>% 
  group_by(Gen_Class) %>% 
...
ggplot(data = lc_rawbats_GenLCsumm, aes(x = reorder(Gen_Class, mean_LC), y = mean_LC)) + 
  geom_point() +
  geom_errorbar(aes(ymin=mean_LC-stdev_LC, ymax=mean_LC+stdev_LC)) +
  coord_flip() + 
  labs(x = "Land Cover", y = "Average Raw Bats Over Land Cover Type") +
  theme_bw() +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
```

```{r}
lc_rawbats_summ <- lc_label_zonalstats %>% 
  st_drop_geometry() %>% 
  group_by(Classification) %>% 
  summarise(mean_LC = mean(mean, na.rm = T), stdev_LC = mean(stdev, na.rm = T))
```
...
```{r}
ggplot(data = lc_rawbats_summ, aes(x = reorder(Classification, mean_LC), y = mean_LC)) + 
  geom_point() +
  geom_errorbar(aes(ymin=mean_LC-stdev_LC, ymax=mean_LC+stdev_LC)) +
  coord_flip() + 
  labs(x = "Land Cover", y = "Average Raw Bats Over Land Cover Type") +
