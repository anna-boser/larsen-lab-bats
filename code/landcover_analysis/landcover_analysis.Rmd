...
bat_exp <- rast(here("data/bats_roost_adjusted/bats_exponential_corrected_2018_postprocess.tif")) 
...
# 
fveg_landIQ_labeled_summ_vect <- vect(fveg_landIQ_labeled_summ) # Still need to assign this
# 
...
```{r}
extract_expBats_terra_NewCode_sum <- cbind(fveg_landIQ_labeled_summ_vect, terra::extract(bat_exp_NAD83, fveg_landIQ_labeled_summ_vect, fun = sum, na.rm=TRUE))
...
extract_expBats_terra_NewCode_sum_df <- extract_expBats_terra_NewCode_sum_df %>% 
  rename(TotalBats = bats_exponential_corrected_2018)
```
...
allpolys_lc_expbats_mean <- ggplot(data = extract_merge_obs_mergecolor, aes(x = reorder(NewClass, Mean), y = Mean, color = Group)) + 
  ...
  labs(x = "Land Cover", y = "Bat Presence (Count)") +
