...
```{r}
# Data IQ crop data from 2018 downloaded from https://data.cnra.ca.gov/dataset/statewide-crop-mapping
landIQ <- st_read("~/Documents/github/larsen-lab-bats/data/landcover_analysis/landIQ_2018/i15_Crop_Mapping_2018.shp") %>% 
...
```{r}
ggplot(data = lc_area_summ, aes(x = reorder(WHR10NAME, area_ha), y = area_ha)) +
  geom_col() + 
  coord_flip() +
...
ggplot(data = lc_bats_summ, aes(x = reorder(WHR10NAME, mean_LC), y = mean_LC)) + 
  geom_point() +
