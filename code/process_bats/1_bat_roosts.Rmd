...
bat_occ_df <- corrected_pred(bat_occ_df, func="exponential_r0.05")
```
...
```
```{r}
far_from_roosts$exponential_r0.05_pred_noneg <- ifelse(far_from_roosts$exponential_r0.05_pred>0, far_from_roosts$exponential_r0.05_pred, 0)
ggplot() + 
  geom_raster(data = far_from_roosts, aes(x=x, y=y, fill=exponential_r0.05_pred_noneg)) + 
  geom_point(data = bat_roosts, aes(x=xcoord, y=ycoord)) + 
  ggtitle("exponential_corrected_bats (0.01 degrees from roosts)") + 
  scale_fill_distiller(palette="Spectral", direction = 1)
```
...
```{r}
# ok what happens if you train it on a much larger area
for (i in 1:nrow(bat_roosts)){
  bat_occ_df <- exponential_pred(d=bat_occ_df, roost_num=i, r=.5)
}
```


```{r}
bat_occ_df <- corrected_pred(bat_occ_df, func="exponential_r0.5")
fwrite(bat_occ_df, here("data", "bats_roost_adjusted", "bat_occ_df.csv"))
```
...
ggplot() + 
  geom_raster(data = far_from_roosts, aes(x=x, y=y, fill=exponential_r0.5)) + 
  geom_point(data = bat_roosts, aes(x=xcoord, y=ycoord)) + 
  ggtitle("exponential_corrected_bats (0.01 degrees from roosts)") + 
  scale_fill_distiller(palette="Spectral", direction = 1)

ggplot() + 
  geom_raster(data = far_from_roosts, aes(x=x, y=y, fill=exponential_r0.5_pred)) + 
  geom_point(data = bat_roosts, aes(x=xcoord, y=ycoord)) + 
  ggtitle("exponential_corrected_bats (0.01 degrees from roosts)") + 
  scale_fill_distiller(palette="Spectral", direction = 1)
...
```
```{r}
far_from_roosts$exponential_r0.5_pred_noneg <- ifelse(far_from_roosts$exponential_r0.5_pred>0, far_from_roosts$exponential_r0.5_pred, 0)
ggplot() + 
  geom_raster(data = far_from_roosts, aes(x=x, y=y, fill=exponential_r0.5_pred_noneg)) + 
  geom_point(data = bat_roosts, aes(x=xcoord, y=ycoord)) + 
  ggtitle("exponential_corrected_bats (0.01 degrees from roosts)") + 
  scale_fill_distiller(palette="Spectral", direction = 1)
```
...
r <- rasterFromXYZ(far_from_roosts[, c('x', 'y', 'exponential_pred')], crs = crs(bat_occ))

# save
writeRaster(r, here("data","aggregated_data", "bats_exponential_corrected_2017.tif"))
```
